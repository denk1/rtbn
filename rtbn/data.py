...
def find_address(place_name, type_place):
    places = AddressItem.objects.filter(address_item_type=type_place, address_item_name__iexact=place_name)
    if places.count() > 0:
        return places[0]
...
def fill_address(address_data, address_type):
    is_new = False
    addr_prev = None
    addr = None
    for i in range(len(address_data)):
        ...
        if addr is None:
            addr = AddressItem.objects.create(address_item_name = address_data[i], address_item_type=address_type[i], above_address_unit=addr_prev)
        addr_prev = addr
    return addr
...
def fill_new_line(post_obj):
    ...
    born_address_type = [int(born_region_type), int(born_district_type), int(born_locality_type)]
    #born address
    ...
    live_locality_type = post_obj.get('live_locality_type')
    live_address = [live_region_name, live_district_name, live_locality_name]
    live_address_type = [int(live_region_type), int(live_district_type), int(live_locality_type)]
    #live address
    ...
    military_enlistment_office_district = fill_address([region_military_enlistment_office,
                                              ...
                                              [int(region_type_military_enlistment_office),
                                               int(district_type_military_enlistment_office),
                                              ])
    
    
    military_enlistment_name = post_obj.get('military_enlistment_office')
    ...
    last_msg_address_types = [int(last_msg_region_type), int(last_msg_district_type), int(last_msg_locality_type)]
    ...
    for i in range(fights_size):
        ...
        war_operation.save()
        war_archievenment = WarArchievement.objects.create(war_operation=war_operation,
                                                            war_serve=war_serve,
    ...
    # ########################
    hospital_name_list = post_obj.getlist('hospital_name[]')
    hospital_period_since_list = post_obj.getlist('hospital_since[]')
    hospital_period_to_list = post_obj.getlist('hospital_to[]')
    hospital_region_list = post_obj.getlist('hospital_region_name[]')
    hospital_region_type_list = post_obj.getlist('hospital_region_type[]')
    hospital_district_list = post_obj.getlist('hospital_distric_name[]')
    hospital_district_type_list = post_obj.getlist('hospital_district_type[]')
    hospital_locality_list = post_obj.getlist('hospital_locality_name[]')
    hospital_locality_type_list = post_obj.getlist('hospital_locality_type[]')
    hospital_army_list = post_obj.getlist('hospital_army_name[]')
    hospital_division_list = post_obj.getlist('hospital_division_name[]')
    hospital_regiment_list = post_obj.getlist('hospital_regiment_name[]')
    healthy_direction_list = post_obj.getlist('healthy_direction[]')
    ...
    for i in range(hospital_size):
        ...
        war_unit_consist = fill_warunit(war_unit_consist_dict)
        hospital_locations = [hospital_region_list[i], hospital_district_list[i], hospital_locality_list[i]]
        hospital_location_types = [int(hospital_region_type_list[i]), int(hospital_district_type_list[i]), int(hospital_locality_type_list[i])]
        hospital_locality = fill_address(hospital_locations, hospital_location_types)
        hospitalization = Hospitalization.objects.create(person=new_person,
                                                         hospital=hospital,
                                                         hospital_location=hospital_locality,
                                                         period_from=hospital_period_since_list[i],
                                                         period_to=hospital_period_to_list[i],
                                                         war_unit_consist=war_unit_consist,
                                                         direction_name=healthy_direction_list[i]
                                                         )
    # ############################
    # The captivity - N5
    # ############################
    date_of_captivity_list = post_obj.getlist('date_of_captivity[]')
    capt_region_list = post_obj.getlist('capt_region[]')
    capt_region_type_list = post_obj.getlist('capt_region_type[]')
    capt_district_list = post_obj.getlist('capt_district[]')
    capt_district_type_list = post_obj.getlist('capt_district_type[]') 
    capt_locality_list = post_obj.getlist('capt_locality[]')
    capt_locality_type_list = post_obj.getlist('capt_locality_type[]')
    camps_list = post_obj.getlist('camp[]')
    ...
    for i in range(count_captivities):
        ...
        capt_address_type = [int(capt_region_type), int(capt_district_type), int(capt_locality_type)]
        ...
        camp_arbeit_list = post_obj.getlist('work_command[][]')
        period_from_list = post_obj.getlist('work_since[][]')
        period_to_list = post_obj.getlist('work_to[][]')
        ...
        infirmary_camp_list = post_obj.getlist('infirmary[][]')
        period_from_list = post_obj.getlist('infir_since[][]')
        period_to_list = post_obj.getlist('infir_to[][]')
    ...
    burial_district_doc = post_obj.get('burial_distric_doc')
    burial_district_type_doc = post_obj.get('burial_distric_doc_type')
    burial_locality_doc = post_obj.get('burial_locality_doc')
    burial_locality_type_doc = post_obj.get('burial_locality_doc_type')
    burial_doc_address_list = [burial_region_doc, burial_district_doc, burial_locality_doc]
    burial_doc_address_types_list = [int(burial_region_type_doc), int(burial_district_type_doc), int(burial_locality_type_doc)]
    ...
    burial_district_type_act = post_obj.get('burial_district_act_type')
    burial_locality_act = post_obj.get('burial_locality_act')
    burial_locality_type_act = post_obj.get('burial_locality_act_type')
    burial_act_address_list = [burial_region_act, burial_district_act, burial_locality_act]
    burial_act_address_types_list = [int(burial_region_type_doc), int(burial_district_type_doc), int(burial_locality_type_doc)]
    burial_act_address = fill_address(burial_doc_address_list, burial_doc_address_types_list)
    #defenition of the burial
    cemetery = post_obj.get('cementary')
    ...
    reburial_region_fact = post_obj.get('reburial_region_fact')
    reburial_region_type_fact = post_obj.get('reburial_region_fact_type')
    reburial_district_fact = post_obj.get('reburial_distric_fact')
    reburial_district_type_fact = post_obj.get('reburial_district_fact_type') 
    reburial_locality_fact = post_obj.get('reburial_locality_fact')
    reburial_locality_type_fact = post_obj.get('reburial_locality_fact_type')
    reburial_fact_address_list = [reburial_region_fact, reburial_district_fact, reburial_locality_fact]
    reburial_fact_address_types_list = [int(reburial_region_type_fact), int(reburial_district_type_fact), int(reburial_locality_type_fact)]
